...
import { TouchableOpacity, useColorScheme, GestureResponderEvent } from 'react-native';
...
// Interface for ButtonAnodiam props
interface ButtonAnodiamProps {
  buttonType?: 'primary' | 'secondary' | 'hyperlink';
  buttonText?: string;
  color?: string;
  ...
  alignItems?: string;
  onPrsBtnAnodiam?: (event: GestureResponderEvent) => void; // Method for handling button press
}
...
//-----------------------------------------------------------------------------------------------
const validateButtonType = (propName: string, propValue: string | undefined, defaultValue: string, filename: string) => {
  if (propValue !== undefined && !VALID_BUTTON_TYPES.includes(propValue)) {
    console.warn(
      `${filename}: Invalid value "${propValue}" passed to prop "${propName}". It should be a valid value from [${VALID_BUTTON_TYPES}]. Defaulting to ${defaultValue}`
    );
  }
};

// Main component: ButtonAnodiam
const ButtonAnodiam: React.FC<ButtonAnodiamProps> = (props) => {
  ...
  const backgroundColor =
    ...
    (buttonType === 'primary'
      ...
      ? colorScheme === 'dark'
        ? Colors.dark.ANODIAM_PALE
        : Colors.light.ANODIAM_LIGHTEST
      : colorScheme === 'dark'
      ? Colors.dark.CONTRAST_PALE
      : Colors.light.ANODIAM_PALE);
