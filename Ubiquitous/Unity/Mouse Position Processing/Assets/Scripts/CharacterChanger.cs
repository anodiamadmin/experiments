...
public class CharacterChanger : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0)) // Check if the left mouse button is pressed down (left button is primary unless changed = 0)
        {
            ...
            // Note: Commented out lines 79, 50-54, 31 and 21 implements a logic to ensure that the same character does not popup twice in a row on mouse click. To implement it, uncomment those lines and comment out the line no. 48 before building the solution.
        }
    }
}
