      ...
      switch (key) {
        ...
        case 'textAlign':
          if (!constants.VALID_TEXT_ALIGNS.includes(value)) {
            raiseIssue('Warning', fileName, key, value, `textAlign from [${constants.VALID_TEXT_ALIGNS}] (wrapped in quotes ''). Defaulting to ${constants.textAlignDefault}`)}
          break
        case 'justifyContent':
          if (!constants.VALID_JUSTIFY_CONTENTS.includes(value)) {
            raiseIssue('Warning', fileName, key, value, `justifyContent from [${constants.VALID_JUSTIFY_CONTENTS}] (wrapped in quotes ''). Defaulting to ${constants.justifyContentDefault}`)}
          break
        case 'alignItems':
...
export const validateBottomNavAnodiamTabs = (tabs, bigIconIndex, defaultSelectedIndex) => {
