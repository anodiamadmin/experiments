...
interface TextInputAnodiamProps {
  textInputType?: typeof constants.textInputTypes[number];
  ...
  maxLength?: number;
  gapVertical?: number;
  onBlurTxtIpAnodiam?: () => void;
  onChngTxtIpAnodiam?: (text: string) => void;
  validateApplicationLogic: (input: string) => void;
}

const TextInputAnodiam = (props:TextInputAnodiamProps) => {
  const [email, setEmail] = useState('');
  const colorScheme = useColorScheme();
  const fileName = 'TextInputAnodiam';

  // textInput Specific Props
  const textInputType = constants.textInputTypes.includes(props.textInputType || '')
    ? props.textInputType!
    : constants.defaultTextInputType;
  const textInputLabel =
    props.textInputLabel ||
    constants.defaultTextInputLabels[constants.textInputTypes.indexOf(textInputType)];
  const mandatory = props.mandatory || false;
  const confirmPasswordRequired =
    props.confirmPasswordRequired === undefined
      ? constants.defaultConfirmPasswordRequired
      : props.confirmPasswordRequired;
  ...
  const [validationText, setValidationText] = useState(
    props.validationText || (mandatory ? constants.defaultMandatoryStarText : constants.defaultValidationText)
  );
  const validationTextColor = props.validationTextColor || Colors.RED;
  const placeholder =
    props.placeholder ||
    'Enter: ' +
    (textInputLabel ? textInputLabel.length > 20 ? textInputLabel.substring(0, 18) + '...' : textInputLabel:'');
  const color = props.color || Colors.ANODIAM;
  const inputColor =
    props.inputColor || (colorScheme === 'dark' ? Colors.dark.ANODIAM_DARKER : Colors.light.ANODIAM_DARKER);
  ...
  const calculatePasswordStrength = (createPswd: string): number | string => {
    if (!createPswd || createPswd.trim() === '') {
      return '';
    }
    let criteriaMet = 0;
    if (/[a-z]/.test(createPswd)) {
      criteriaMet += 1;
    }
    if (/[A-Z]/.test(createPswd)) {
      criteriaMet += 1;
    }
    if (/[0-9]/.test(createPswd)) {
      criteriaMet += 1;
    }
    if (/[!@#$%^&*()_+={}|:;"'<>,.?/\\₹£¥€₩฿₿₫₺؋֏៛₾₦₱₲₭₮]/.test(createPswd)) {
      criteriaMet += 1;
    }
    if (createPswd.length >= constants.MIN_PASSWORD_LENGTH) {
      criteriaMet += 1;
    }
    return criteriaMet;
  };
  ...
  const [confPswdValidationTextColor, setConfPswdValidationTextColor] = useState<string>(validationTextColor);
  const [confPswdValidationFontSize, setConfPswdValidationFontSize] = useState<number>(fontSize);

  const confPswdSuccess = (passwordStrength: number, confirmPassword: string) => {
  ...
  const confPswdFailure = (passwordStrength: number, confirmPassword: string) => {
    if (passwordStrength >= 5 && confirmPassword.length >= constants.MIN_PASSWORD_LENGTH) {
  ...
  const createPswdTextChange = (createPswd: string) => {
    ...
    if (typeof passwordStrength === 'number') {
      setCreatePswdStrength(passwordStrength);
      if (passwordStrength >= 5) {
        setValidationText('');
        createPswd === confirmPassword
          ? confPswdSuccess(passwordStrength, confirmPassword)
          : confPswdFailure(passwordStrength, confirmPassword);
      }
    }
  };
  ...
  const createPswdOnBlur = () => {
    if (createPswdStrength < 5 && createPassword !== '') {
      setValidationText(
        `Minimum ${constants.MIN_PASSWORD_LENGTH} characters: lowercase, uppercase,\n numeric & special chars.`
      );
    } else {
      setValidationText('');
    }
  ...
  const confPswdOnBlur = () => {
    if (createPswdStrength >= 5) {
      confirmPassword === createPassword
        ? confPswdSuccess(createPswdStrength, confirmPassword)
        : confPswdFailure(createPswdStrength, confirmPassword);
    }
  };

  const confPswdTextChange = (confPswd: string) => {
    setConfirmPassword(confPswd);
    if (createPswdStrength >= 5) {
      confPswd === createPassword
        ? confPswdSuccess(createPswdStrength, confPswd)
        : confPswdFailure(createPswdStrength, confPswd);
    }
    if (confPswd.length < constants.MIN_PASSWORD_LENGTH) {
      setConfPswdValidationText('');
    }
  };
    ...
    case 'create-password':
      content = (
        <View>
          ...
          {createPswdStrength ? <PasswordStrengthMeter createPswdStrength={createPswdStrength} /> : null}
